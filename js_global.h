const char HTTP_GLOBALSCRIPT[] PROGMEM = R"=====(
var timerRun = false; 
...
/*Aktualiesierung der ESP Zeit auf html Seite */
setTimeout(function(){ refreshTime(); }, 500); 
function refreshTime() { 
  var json_obj = JSON.parse(Get('/getTimeJSON')); 
  document.getElementById('aDatumZeit').innerHTML = json_obj.joDatumZeit; 
  setTimeout(function(){ refreshTime() }, 500); 
}
